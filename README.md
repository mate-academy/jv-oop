...
- Create classes `Truck`, `core.basesyntax.Truck.Bulldozer`, `Excavator` that will inherit from `Machine`.
...
- In `MainApp` class create `Machine` array with `Truck`, `core.basesyntax.Truck.Bulldozer` and `Excavator` and call methods `doWork()` and `stopWork()` in a loop.
